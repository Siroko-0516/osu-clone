...
test('input bridge preserves short taps, focus release, and scaled pointer coordinates', () => {
    ...
    try {
        ...
        emit(window, 'keydown', {code:'KeyZ',repeat:true});
        ...
        emit(window, 'keyup', {code:'KeyX'});
        assert.deepEqual(bridge.drain(), [
            {kind:'key',code:'KeyX',pressed:true},
            {kind:'key',code:'KeyX',pressed:false}
        ], 'gameplay keys remain active after focus leaves the canvas');
        ...
        emit(window, 'keydown', {code:'KeyD'});
        emit(window, 'keydown', {code:'KeyF'});
        emit(window, 'blur', {});
